...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Modifier un Magasin', href: '#' },
]
...
const form = useForm({
    id_magasin: props.magasin.id_magasin,
    adresse_magasin: props.magasin.adresse_magasin,
    id_centre: props.magasin.id_centre,
})

function submit() {
    form.put(`/gestionmagasin/${props.magasin.id_magasin}`)
}
</script>

<template>
    <Head title="Modifier un Magasin" />
            ...
            <form @submit.prevent="submit" class="space-y-6 bg-white dark:bg-gray-700 p-6 rounded-lg shadow">
                <div class="space-y-2">
                    ...
                    <input
                        ...
                        type="number"
                        disabled
                        class="w-full bg-gray-100 dark:bg-gray-900 border border-gray-300 dark:border-gray-600 p-2 rounded text-gray-500 dark:text-gray-400 cursor-not-allowed"
                    />
                </div>

                <div class="space-y-2">
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Adresse Magasin</label>
                    <input
