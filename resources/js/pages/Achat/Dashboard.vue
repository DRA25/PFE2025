...
import { ClipboardCheck, FolderSearch, PackageSearch, ShoppingCart, BarChart3, ReceiptText } from 'lucide-vue-next'; // Import relevant icons
...
// Map paths to appropriate icons for better semantic representation for Achat
const getIconForPath = (path: string) => {
    if (path.includes('achat/dras')) return FolderSearch; // Icon for Consulter les DRAs
    // Add other Achat-specific icons if more links are added in the future
    // For example:
    // if (path.includes('achat/commandes')) return ShoppingCart;
    // if (path.includes('achat/factures')) return ReceiptText;
    // if (path.includes('achat/rapports')) return BarChart3;
    return FolderSearch; // Default icon if no specific match
};
</script>

<template>
    <Head title="Achat Dashboard" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 lg:gap-10">
                        ...
                        >
                            <div class="p-4 bg-[#042B62] text-white rounded-full mb-4 group-hover:bg-[#F3B21B] group-hover:text-[#042B62]
                                ...
                                <component :is="getIconForPath('achat/dras')" class="w-8 h-8 sm:w-9 sm:h-9" />
                            </div>
                            ...
                            <p class="text-gray-600 dark:text-gray-300 text-sm sm:text-base leading-relaxed">
                                Visualisez et gérez toutes les Demandes de Réquisition d'Achat.
                            </p>
                        </Link>
                    </div>
                </div>
